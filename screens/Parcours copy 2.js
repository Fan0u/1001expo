...
const LOCATION_TASK_NAME = 'location-gps-task';

const requestPermissions = async () => {
	const { status } = await Location.requestForegroundPermissionsAsync();
	if (status === 'granted') {
	  await Location.watchPositionAsync(LOCATION_TASK_NAME, {
...
const Cordinate = ({ navigation }) => {
	...
	const gpsAsync = async () => {
		...
		console.log("LocationUpdateAsync");
	...
	return (
		<View style={styles.container}>
			...
			<View>
			<TouchableOpacity onPress={gpsAsync}>
				<Text>{region.latitude} / {region.longitude}</Text>
			</TouchableOpacity>
			</View>
		</View>
...
// TaskManager.defineTask(GPS_TRACKER, ({ data: { locations }, error }) => {
// 	if (error) {
